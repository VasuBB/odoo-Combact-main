...
const askMCQ = async ()=>{
  try {
    const response = await fetch('https://beta-free.onrender.com/query', {
        ...
        body: JSON.stringify({ query: `
          based on the full content 
          provide 10 mcq questions 
          in the following json format with below schema
          just give the string do not marked code just give string
          give in string fomat only !!
...
const genMCQ = async () => {
  ...
  bokadoka(true);
  console.log('button clicked');
  let test = await askMCQ();
  try {
    jsonObject = JSON.parse(test);
    console.log(jsonObject);

    const url = "http://localhost:7000/quiz/add_quiz"
